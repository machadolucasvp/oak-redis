...
class ProductRepository implements Repository<number, Product> {
  ...
  async get(identifier: number): Promise<Product>{
    const result = await Database.query({
      text: `select id, name, price, barcode, description from products where id=$1;`,
      args: [identifier],
    });
  ...
  async post(product: Product): Promise<Product>{
    const result = await Database.query({
      text:
        "insert into products (id, name, price, barcode, description) values ($1, $2, $3, $4, $5) returning id, title, isbn;",
      args: [
        product.id,
        product.name,
        product.price,
  ...
  async delete(identifier: number): Promise<Product>{
    const result = await Database.query({
      text:
        "delete from products where id=$1 returning id, name, description;",
      args: [
        identifier
      ],
    });
